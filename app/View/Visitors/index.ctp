...
<?php
		...
		array('controller'=>'endscopes','action'=>'index',$visdata[0]['latest']));
?><||>
<?php
...
<li><big><?php echo h($visitor['created']);?></big>
<?php
	if($visitor['Carte']){
		echo $this->Html->link('編集',
		array('controller'=>'cartes','action'=>'edit',$visitor['Carte']['id']));
	}else{
		echo $this->Html->link('カルテ記載',
		array('controller'=>'cartes','action'=>'add',$visdata[0]['latest']));
	}
...
記載無し
<?php }else{;?>
<li>S:<?php echo h($visitor['Carte']['subject']);?></li>
<li>O:<?php echo h($visitor['Carte']['object']);?></li>
